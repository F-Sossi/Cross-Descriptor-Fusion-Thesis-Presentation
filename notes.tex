...
\begin{itemize}
  ...
  \item Different families have different strengths, and that's our opportunity.
...
\begin{itemize}
  ...
  \item In L2 distance, SIFT completely dominates and HardNet contribution is invisible.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Algorithm detail, Mutual Nearest Neighbor (MNN) with spatial tolerance:}
    ...
    \end{itemize}
  \item \textbf{Only spatial position $(x, y)$ is used for matching.} Scale and orientation are NOT matching criteria. Rationale: SIFT averages 4.45\,px while KeyNet averages 49.83\,px, so requiring scale agreement would eliminate most correspondences.
...
\begin{itemize}
  ...
  \item 4px samples 16$\times$16, 10px samples 40$\times$40, a huge difference.
\end{itemize}
...
\begin{itemize}
  \item The +18\% mAP gain from intersection was surprising, so we needed to rule out simpler explanations. We created control sets: random subsets, top-N by response, top-N by scale, and spatially filtered sets, all matched to intersection count (645K keypoints).
  \item \textbf{NOT better repeatability:}
    \begin{itemize}
      \item Intersection keypoints actually have \textbf{lower} repeatability: 28.0\% vs 29.4\% for scale-controlled.
      \item Paired t-test: $t = -4.28$, $p < 0.0001$, a statistically significant \textbf{decrease}.
      \item Yet matching performance is \textbf{higher}. This means repeatability alone does not explain the gain.
    \end{itemize}
  \item \textbf{NOT more distinctive descriptors:}
    \begin{itemize}
      \item Compared nearest-neighbor distance ratios for correct matches only.
      \item Both sets: NN ratio $\approx 0.44$, identical.
      \item The same HardNet model produces the same descriptor for a given patch regardless of how the location was selected. The descriptors are equally distinctive; the difference is \textbf{which keypoints} are selected.
    \end{itemize}
  \item \textbf{NOT higher response values:}
    \begin{itemize}
      \item Both intersection and scale-controlled sets: average response $\approx 0.035$.
      \item Keypoint ``strength'' is identical; the detector confidence scores do not differ.
    \end{itemize}
  \item \textbf{NOT spatial crowding:}
    \begin{itemize}
      \item Quadrant analysis shows nearly identical spatial distributions: TL 13\%, TR 18\%, BL 24\%, BR 45\%.
      \item Same distribution for both sets, so filtering is uniform across the image, not biased toward any region.
    \end{itemize}
\end{itemize}
...
\begin{itemize}
  \item \textbf{Confirmed mechanism:} Detector consensus selects locations where local image structure is inherently unique, significant enough to trigger detection in both a hand-crafted (SIFT) and learned (KeyNet) detector. These locations avoid repetitive textures (bricks, tiles, windows) where many keypoints look similar.
  \item \textbf{Higher precision:}
    \begin{itemize}
      \item At threshold 0.8: intersection 71.2\% vs scale-controlled 66.2\% (+5.0 percentage points).
      \item Fewer ambiguous matches means better ranking of correct correspondences.
    \end{itemize}
  \item \textbf{Fewer false positives:}
    \begin{itemize}
      \item Intersection: 0.40 FP per TP vs scale-controlled: 0.51 FP per TP.
      \item 22\% reduction in false positives because repetitive-texture keypoints that confuse matching are removed.
    \end{itemize}
  \item \textbf{All descriptors benefit equally:}
    \begin{itemize}
      \item DSPSIFT +28\%, RGBSIFT +31\%, HoNC +32\%, HardNet +17\%, SOSNet +17\%.
      \item If the improvement were descriptor-specific, we'd see uneven gains. Uniform improvement proves the gain comes from \textbf{location quality}, not descriptor quality.
    \end{itemize}
  \item \textbf{Why detector agreement indicates quality:}
    \begin{itemize}
      \item Cross-validation: if both a blob detector (KeyNet) and an edge/corner detector (SIFT) independently identify the same location, it likely corresponds to real image structure.
      \item Noise rejection: detector-specific false detections are unlikely to coincide across different methods.
      \item Practical impact: keypoint selection strategy can matter as much as descriptor algorithm choice. The 17\% gain from intersection is comparable to the gap between SIFT and CNN descriptors.
    \end{itemize}
\end{itemize}
...
\begin{itemize}
  \item Go through each RQ deliberately. Pause between each one.

  \item \textbf{RQ1: Does detector consensus provide a keypoint quality signal?}\\
    Yes. Detector intersection improved HardNet from 64.5\% to 82.1\% mAP (+18\%). We validated that this is not repeatability, distinctiveness, response strength, or spatial bias. Consensus keypoints avoid repetitive textures where matching is ambiguous. The gain is comparable to switching from SIFT to a CNN descriptor.

  \item \textbf{RQ2: Can color descriptors improve fusion?}\\
    Yes. HoNC + SOSNet concatenation achieved 50.6\% mAP on the color patch benchmark, outperforming SOSNet alone at 48.9\%. HoNC acts as a discriminator (V/M ratio 3.84$\times$), adding complementary rejection power that the CNN matcher lacks. This was only possible because we built the color patch benchmark.

  \item \textbf{RQ3: What compatibility patterns govern descriptor fusion?}\\
    Complementarity determines success. Same-family fusion fails (SIFT + RGBSIFT = 10.4\%, worse than either alone) because correlated features add noise. Cross-family fusion requires magnitude matching. Without L2 normalization, SIFT (0 to 512) dominates HardNet ($-0.3$ to $+0.3$) in L2 distance. The discriminator-matcher framework predicts outcomes: pair a high V/M descriptor with a low V/M descriptor for best results.

  \item \textbf{RQ4: How does keypoint scale impact performance?}\\
    Scale is a dominant factor. Filtering to top 25\% by scale: SIFT 44.5\% $\to$ 62.8\% (+18.3\%), HardNet 64.5\% $\to$ 78.1\% (+13.6\%). These gains are comparable to switching descriptor families. Larger keypoints sample more image context. A 4\,px keypoint sees about 16$\times$16 pixels while a 10\,px keypoint sees about 40$\times$40. Quality over quantity.

  \item All four questions answered with quantitative evidence from both pipelines.
\end{itemize}
...
\begin{itemize}
  \item Short-term: systematic study of fusion weighting. We used equal weighting ($\alpha = 0.5$) throughout; varying this could improve results. Also more descriptors (DISK, ALIKE, SuperPoint).
