...
\usepackage{xcolor}
\usepackage{tikz}
\usepackage{pifont}  % For \ding symbols (checkmarks, X marks)
\usepackage{pdfpc}
...
\begin{frame}[plain]
  \begin{tikzpicture}[remember picture, overlay]
    ...
    \node[anchor=north west, text width=0.75\paperwidth, font=\large, text=white!85]
      ...
      {Scale Control and Spatial Alignment\\for Local Feature Matching};
    % Author info
    \node[anchor=south west, text width=0.9\paperwidth, font=\small, text=white!90]
      at ([shift={(1.5cm,0.6cm)}]current page.south west)
      {\normalsize Frank Sossi\\[3pt]
       School of STEM, University of Washington Bothell\\[3pt]
       {\itshape Committee: Prof.\ Clark Olson (Chair),
        Prof.\ Min Chen, Prof.\ Dong Si}\\[3pt]
       2026};
  \end{tikzpicture}
...
\begin{frame}{Thesis in One Sentence}
  ...
  \begin{beamercolorbox}[sep=1em, rounded=true, shadow=true, center]{block body}
    \large
    Selecting high-quality keypoints through detector consensus and fusing complementary descriptors yields large improvements in local feature matching.
  \end{beamercolorbox}
  \vfill
...
\begin{frame}{Outline}
  \tableofcontents
  \pdfpcnote{Four-part structure, about 35--40 minutes then questions. FRAMING: the problem, state of the art, key concepts (detection/description/matching pipeline, mAP), four research questions, and who benefits. PROCESS: two evaluation pipelines (full-image vs patch benchmark), DescriptorWorkbench architecture, building the color patch benchmark, the spatial intersection algorithm, scale characteristics of detectors, and three key findings---same-family fusion fails, cross-family fusion needs normalization, keypoint quality rivals descriptor choice. RESULTS: baselines, scale control impact (+18\% SIFT, +14\% HardNet), detector intersection progression to 82\%, two slides validating the intersection mechanism, CNN+CNN fusion to 93.4\%, the discriminator-matcher framework, patch benchmark fusion results, and answering all four RQs. LESSONS LEARNED: technical insights, engineering lessons, future work, limitations, and summary of six contributions.}
\end{frame}
...
\begin{frame}{State of the Art When This Work Began}
    \pdfpcnote{LEFT SIDE---what existed: SIFT is the classic, 128-D gradient histograms from 2004. SURF is faster with Haar wavelets, 64-D. HardNet and SOSNet are CNN descriptors trained on patch triplets---they learn what makes patches similar. HoNC from Prof. Olson captures color surface normal histograms---unique because it uses color information that all the others discard. RIGHT SIDE---the gaps: nobody had systematically studied what happens when you fuse descriptors across families. No understanding of when fusion helps versus hurts. The role of keypoint quality---scale and detector agreement---was not well studied. And the original HPatches benchmark only provides grayscale patches, so color descriptors like HoNC couldn't be fairly evaluated. These four gaps define our contribution.}
  \begin{columns}[T]
    \column{0.5\textwidth}
    \textbf{What existed:}
    \begin{itemize}
...
\begin{frame}{Key Concepts}
  \centering
  \begin{tikzpicture}[
      box/.style={draw, rounded corners, minimum height=1cm, minimum width=2.4cm,
                  font=\small, fill=uwpurple!10},
      arr/.style={->, thick, >=stealth}
    ]
    \node[box] (det) at (0,0)   {Detection};
    \node[box] (desc) at (3.5,0)  {Description};
    \node[box] (match) at (7,0) {Matching};
  ...
  \end{columns}
  \pdfpcnote{Walk through the pipeline left to right. 
  DETECTION: find salient locations---corners, blobs---that are likely to reappear if the camera moves or lighting changes. 
  
  DESCRIPTION: at each keypoint, sample a patch and encode it as a fixed-length vector. Matching is then just nearest-neighbor search in descriptor space. 
  
  mAP explained: for each query descriptor, rank all candidates by distance. Average Precision is the area under the precision-recall curve for that query---it rewards ranking the correct match high. Mean AP averages this over all queries across all image pairs. 50\% mAP means on average the correct match is ranked in the top half; 90\% means it's almost always at the top. We use the Bojanic et al. evaluation protocol which tests across 116 HPatches sequences with ground-truth homographies.}
\end{frame}
...
\begin{frame}{Research Goals \& Measures of Success}
  ...
  \end{enumerate}
  \vfill
  {\small Evaluated on a color re-implementation of the HPatches benchmark~\cite{balntas2017hpatches} and a full image pipeline based on the original HPatches source image set.}
  \pdfpcnote{Four research questions, each with a concrete success criterion. RQ1: detector consensus. RQ2: color fusion. RQ3: compatibility patterns. RQ4: scale impact. }
\end{frame}
...
\begin{frame}{DescriptorWorkbench Architecture}
  \begin{columns}[T]
    ...
    \begin{itemize}
      \item 10 descriptor types (SIFT, RGBSIFT, HoNC, HardNet, SOSNet, \ldots)
    ...
    \vspace{0.5em}
    \textbf{Open Source}
      All code, data, and results are available on GitHub.
  \end{columns}
  \pdfpcnote{C++ framework, 10 descriptors, YAML-driven. SQLite database tracks all 100+ experiments. Open source on GitHub.}
\end{frame}
...
\begin{frame}{Two Evaluation Protocols}
  \vspace{-0.5em}
  \begin{columns}[T]
    \column{0.5\textwidth}
    \begin{block}{\small Full-Image (Bojanic et al.~\cite{bojanic2020comparison})}
      {\small
      \begin{itemize}\setlength{\itemsep}{0pt}
        \item \textbf{Matching} --- mAP, SNN ratio test
        \item \textbf{Verification} --- AUC, cross-seq.\ distractors
        \item \textbf{Retrieval} --- AP, three-tier labels
      \end{itemize}}
    \end{block}
    \column{0.5\textwidth}
    \begin{block}{\small Patch Benchmark (Balntas et al.~\cite{balntas2017hpatches})}
      {\small
      \begin{itemize}\setlength{\itemsep}{0pt}
        \item \textbf{Matching} --- mAP, descriptor distance
        \item \textbf{Verification} --- AP, same + cross-seq.\ neg.
        \item \textbf{Retrieval} --- mAP, binary labels
      \end{itemize}}
    \end{block}
  \end{columns}
  \vfill
  {\small Both report HP-V (viewpoint) and HP-I (illumination) breakdowns across 116 HPatches sequences.}
  \pdfpcnote{Two separate evaluation protocols. Full-image uses Bojanic et al.: three tasks---matching, verification, retrieval. Tests detector AND descriptor jointly. Patch benchmark uses original HPatches protocol from Balntas et al.: same three tasks but on pre-extracted patches. Key differences: Bojanic uses cross-sequence distractors only and three-tier retrieval labeling; HPatches uses same+cross-sequence negatives and binary labeling. Both break down by viewpoint vs illumination.}
\end{frame}

% --- Patch benchmark methodology ---
\begin{frame}{Patch Pipeline: Building a Color Patch Benchmark}
  \begin{itemize}
...
\begin{frame}{Same-Family Fusion Does Not Help}
  \begin{columns}[T]
    \column{0.55\textwidth}
    \textbf{Hypothesis:} SIFT + RGBSIFT fusion should add color information to grayscale SIFT.
    \vspace{0.5em}

    \textbf{Result (patch benchmark):}
    \begin{table}[h]
    \centering\small
    \begin{tabular}{lr}
      \toprule
      ...
      RGBSIFT alone & 24.6\% \\
      SIFT alone & 22.9\% \\
      SIFT + RGBSIFT concat & {\color{degrade}10.4\%} \\
      \bottomrule
    \end{tabular}
    \end{table}
    ...
    \begin{alertblock}{Why it failed}
      ...
      Fusion doubles dimensionality, adding noise rather than signal.
    \end{alertblock}
  \end{columns}
  \pdfpcnote{Patch benchmark isolates descriptor effects. SIFT alone: 22.9\%, RGBSIFT alone: 24.6\%. Fusion: 10.4\%---worse than either alone. Why? Correlated gradient info. Doubling dimensions adds noise, not signal.}
\end{frame}

\begin{frame}{Cross-Family Fusion Requires Magnitude Matching}
  \begin{columns}[T]
    \column{0.5\textwidth}
    \textbf{Problem:} SIFT + HardNet fusion performs \textbf{worse} than either descriptor alone.
    \vspace{0.5em}
    ...
    \begin{itemize}\setlength{\itemsep}{2pt}
      ...
      \item In L2 distance, SIFT \textbf{dominates}
    \end{itemize}
    ...
    \end{table}
  \end{columns}
  \pdfpcnote{SIFT + HardNet fusion fails without normalization. Root cause: magnitude mismatch. SIFT values go up to 512, HardNet stays around 0.3. In L2 distance, SIFT completely dominates---HardNet contribution is invisible.}
\end{frame}

\begin{frame}{Solution: Pre-Fusion L2 Normalization}
...
% --- Full-image pipeline methodology ---
...
\begin{frame}{Full image pipeline: Scale Characteristics}
  \begin{columns}
    \column{0.5\textwidth}
...
\begin{frame}{Keypoint Quality Matters As much as Descriptor Choice}
  \begin{itemize}
    \item Descriptor research focuses on better \textbf{encoding algorithms}
    \item Yet \textbf{keypoint selection} has an equal or greater effect on performance
  \end{itemize}
  \vfill
  \begin{table}[h]
  \centering
  ...
  \pdfpcnote{Descriptor research focuses on encoding algorithms. But keypoint selection matters just as much. Table shows: switching SIFT to HardNet = +20\%, scale filtering SIFT = +18\%, intersection on HardNet = +18\%. These are comparable gains.}
\end{frame}
...
\begin{frame}{Result: Detector Intersection Progression}
  \begin{columns}
    ...
    {\small Answers \textbf{RQ1}: Yes, intersection keypoints are more distinctive keypoints~\cite{barroso2019key}.}
  \end{columns}
...
\begin{frame}{Validating the Intersection Mechanism (1/2)}
  \textbf{The +18\% mAP gain required verification.} We ruled out four alternatives:
  \begin{columns}[T]
    \column{0.5\textwidth}
    \textbf{\color{degrade}\ding{55} Not better repeatability}\\
    {\small Intersection: 28.0\% vs Scale: 29.4\%\\($p < 0.0001$). Less repeatable, yet better matches.}
    \vspace{0.6em}

    \textbf{\color{degrade}\ding{55} Not more distinctive descriptors}\\
    {\small Correct-match NN ratios identical ($\sim$0.44).\\Descriptors equally good; locations differ.}

    \column{0.5\textwidth}
    \textbf{\color{degrade}\ding{55} Not higher response values}\\
    {\small Both sets: avg response $\sim$0.035.\\Keypoint strength is identical.}
    \vspace{0.6em}

    \textbf{\color{degrade}\ding{55} Not spatial crowding}\\
    {\small Quadrant distributions identical:\\TL 13\%, TR 18\%, BL 24\%, BR 45\%.\\Filtering is uniform across image.}
  \end{columns}
  \pdfpcnote{Sources: (1) Repeatability: analysis/output/descriptor_reports/investigation_2_repeatability_tracking.md. (2) NN ratios: investigation_1_nn_ratio_analysis.md. (3) Response: intersection_mechanism_analysis.md Finding 1. (4) Spatial: analyze_keypoint_properties.py, intersection_mechanism_analysis.md Appendix A.4.}
\end{frame}

\begin{frame}{Validating the Intersection Mechanism (2/2)}
  \textbf{Confirmed:} Detector consensus removes ``confusing'' keypoints at repetitive textures.
  \begin{columns}[T]
    \column{0.5\textwidth}
    \textbf{\color{improve}\ding{51} Higher precision}\\
    {\small At threshold 0.8: 71.2\% vs 66.2\%}
    \vspace{0.6em}

    \textbf{\color{improve}\ding{51} Fewer false positives}\\
    {\small 0.40 vs 0.51 FP per TP (22\% reduction)}
    \vspace{0.6em}

    \textbf{\color{improve}\ding{51} All descriptors benefit equally}\\
    {\small Proves it's \textbf{location quality}.}

    \column{0.5\textwidth}
    {\small\textbf{Multi-descriptor gains over baseline:}}
    \begin{table}[h]
    \centering\small
    \begin{tabular}{lr}
      \toprule
  ...
  \end{columns}
  \vspace{0.3em}
  {\small \textbf{Mechanism:} Locations where both SIFT and KeyNet detect keypoints have unique local structure.}
  \pdfpcnote{Sources: Precision/FP: investigation_1_nn_ratio_analysis.md. Multi-descriptor: intersection_mechanism_analysis.md Finding 7. Key insight: if ALL descriptors benefit equally, the locations are better, not the descriptors. Summary: analysis/output/descriptor_reports/intersection_mechanism_analysis.md.}
\end{frame}
...
\begin{frame}{Result: Patch Benchmark Fusion}
  \vspace{-0.5em}
  \centering
  \includegraphics[width=0.55\textwidth]{figures/patch_benchmark_overview.pdf}
  \vspace{-0.5em}
  \begin{itemize}\setlength{\itemsep}{0pt}\small
    \item \textbf{Best:} HoNC + SOSNet concat = \textbf{50.6\%} mAP (color + learned)
    \item CNN + CNN concat = 49.9\% (complementary learned)
    \item SIFT + HoNC concat = 15.5\% {\color{degrade}(two weak matchers --- hurts)}
  \end{itemize}
...
\begin{frame}{Answering the Research Questions}
  \begin{enumerate}
    \item[\textbf{RQ1}] \textbf{Yes} --- detector intersection improves HardNet by +18\% mAP.
          Consensus keypoints are more distinct.
...
\begin{frame}{Future Work}
  \begin{columns}[T]
    ...
    \textbf{Short-term:}
    \begin{itemize}
      \item Systematic study of fusion weighting strategies and their effect on matching performance
    ...
    \end{itemize}
  \end{columns}
  \pdfpcnote{Short-term: systematic study of how different fusion weightings affect results---we used equal weighting throughout, varying this could improve performance. Also more descriptors like DISK, ALIKE, SuperPoint. Long-term: end-to-end learning, other datasets, real-time deployment. Plenty of directions to extend this work.}
\end{frame}
